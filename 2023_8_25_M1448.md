...
其实可以直接进行递归，而不是使用回溯的想法，如下，在类的参数列表中使用默认的参数来维护当前的节点的路径上的最大值。
...
class Solution {
public:
    int goodNodes(TreeNode* root,int value = -INT_MAX) {
        
        if(!root) return 0;

        return goodNodes(root->left,max(value,root->val)) + goodNodes(root->right,max(value,root->val)) + (value <= root->val);
    }
};
```
